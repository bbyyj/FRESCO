...
def prepare_dift_matching(model_key, imgs,device='cuda'):
    dift = SDFeaturizer(model_key)
    H = (math.ceil(512 / 64), math.ceil(512 / 32), math.ceil(512 / 16),
             math.ceil(512 / 8))
    W = (math.ceil(512 / 64), math.ceil(512 / 32), math.ceil(512 / 16),
             math.ceil(512 / 8))
    ft_list = []
    matching = {}
    key_matching = {}
    for i in tqdm(range(len(imgs)), desc='Extracting DIFT features'):
        ft = dift.forward(numpy2tensor(imgs[i]), prompt='', t=0, up_ft_index=[2], ensemble_size=8)  # [1, c, h, w]
        ...
        '''''
        #if len(imgs) > self.config["max_kv_size"]:
        #    keep_ratio = self.config["max_kv_size"] / len(imgs)
    ...
    for (h, w) in tqdm(zip(H, W), desc='Matching DIFT features', total=len(H)):
        ...
        frames_ft = torch.cat(ft_list)  # [n_frames, c, h, w]
        frames_ft = frames_ft.to(torch.float32)
        ...
        key_match = get_matching(frames_ft, 0, len(frames_ft), is_key=True, device=device)
        k = int(key_match.shape[1] // 2)
        key_matching[f'{h*w}'] = torch.topk(key_match, k, dim=1, largest=True).indices.detach().cpu()
        # if t == 981 and h == 64 and w == 64:
        #     draw_kv(h, w, key_matching[f'{h}_{w}_{t}'], self.config["output_path"], self.frames[all_pivotal_idx[t]])
    torch.cuda.empty_cache()
    return key_matching

def get_matching(frames_ft, begin_idx, batch_size, is_key=False, device='cuda'):
    dim = frames_ft.shape[-1]
    if batch_size <= 10:
        ...
        batch_ft = batch_ft / batch_ft.norm(dim=-1, keepdim=True)
        idx = []
        small_batch_size = 10 // batch_size
...
class AdaFlow(nn.Module):
    ...
    def prepare_dift_matching(self, model_key, imgs,device='cuda'):
        dift = SDFeaturizer(model_key)
        H = (math.ceil(512 / 64), math.ceil(512 / 32), math.ceil(512 / 16),
             math.ceil(512 / 8))
        W = (math.ceil(512 / 64), math.ceil(512 / 32), math.ceil(512 / 16),
        ...
        key_matching = {}
        for i in tqdm(range(len(imgs)), desc='Extracting DIFT features'):
            ft = dift.forward(imgs[i], prompt='', t=0, up_ft_index=[2], ensemble_size=8)  # [1, c, h, w]
            ft_list.append(ft[2].detach().cpu())
        ...
        for (h, w) in tqdm(zip(H, W), desc='Matching DIFT features', total=len(H)):
            ...
            #        t = int(t)
            #        key_frames_ft = frames_ft[all_pivotal_idx[t]]
            key_match = self.get_matching(frames_ft, 0, len(frames_ft), is_key=True, device=device)
            k = int(key_match.shape[1] // 2)
            key_matching[f'{h*w}'] = torch.topk(key_match, k, dim=1, largest=True).indices.detach().cpu()
                    # if t == 981 and h == 64 and w == 64:
                    #     draw_kv(h, w, key_matching[f'{h}_{w}_{t}'], self.config["output_path"], self.frames[all_pivotal_idx[t]])
