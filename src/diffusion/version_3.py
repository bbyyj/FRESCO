    ...
    * adding cross-frame attention
    ...
    def __call__(
        ...
        if self.controller and self.controller.use_cfattn and (not crossattn):
            ...
            warp1=hidden_states[0,0].unsqueeze(0).repeat(bwd_flows[0].shape[0],1,1,1)
            ...
            replace_hidden_states = hidden_states[0,-bwd_flows[0].shape[0]:]
            replace_hidden_states[attn_mask[0]]=warp_hidden_states[attn_mask[0]].half()
            for j in range(hidden_states.shape[0]):
                hidden_states[j,-bwd_flows[0].shape[0]:]=replace_hidden_states
